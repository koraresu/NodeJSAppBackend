		...
		if(status){
			Tokenfunc.toProfile(tokenData.generated_id, function(status, userData, profileData, profileInfoData){
				...
				Profilefunc.logs(profileData, 8, {profile: profileData, search: text }, function(){
					if(status){
						Profile.find({ _id: { "$ne": actualData._id }}).find({ _id: { "$ne": mongoose.Types.ObjectId("57b237e57a28f01f332e3447") }}).populate('experiences').populate('skills').populate('user_id','-password').exec(function(errProfile, profileData){
